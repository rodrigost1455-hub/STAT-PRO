...
function GRRModule({onSaveHistory}) {
  ...
  const computar=()=>{
    ...
    const kappa=(overallPct/100-.5)/.5;
  ...
  const exportPDF=()=>{
    ...
    const html=pdfTemplate({
      ...
      body:`
        ...
        <p>El proceso <strong>"${meta.proceso}"</strong> presenta una proporción promedio defectiva de 
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"320px 1fr",gap:24,padding:24,height:"calc(100vh - 64px)",overflow:"auto"}}>
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div style={{background:CORP_COLORS.card,border:`1px solid ${CORP_COLORS.border}`,borderRadius:10,padding:20}}>
          <SectionTitle>Configuración de Datos</SectionTitle>
        ...
        {result&&<>
          ...
          <div style={{background:CORP_COLORS.card,border:`1px solid ${CORP_COLORS.border}`,borderRadius:10,padding:20}}>
            ...
            <ResponsiveContainer width="100%" height={320}>
              <ComposedChart data={result.chartData} margin={{top:10,right:20,bottom:20,left:10}}>
                ...
                <XAxis dataKey="id" tick={{fill:CORP_COLORS.muted,fontSize:10}}/>
                ...
                <Line type="monotone" dataKey="ucl" stroke={CORP_COLORS.red} strokeDasharray="6 3" strokeWidth={1.5} dot={false} name="UCL"/>
                <Line type="monotone" dataKey="lcl" stroke={CORP_COLORS.red} strokeDasharray="6 3" strokeWidth={1.5} dot={false} name="LCL"/>
          ...
          <div style={{background:CORP_COLORS.card,border:`1px solid ${CORP_COLORS.border}`,borderRadius:10,padding:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
              {[
                {label:"Regla 1 – Punto fuera ±3σ",count:result.flags.filter(f=>f.oc).length,pass:!result.flags.some(f=>f.oc)},
                {label:"Regla 2 – 7 consecutivos",count:result.flags.filter(f=>f.run7).length,pass:!result.flags.some(f=>f.run7)},
                {label:"Regla 3 – Tendencia 6 pts",count:result.flags.filter(f=>f.trend).length,pass:!result.flags.some(f=>f.trend)},
              ].map(({label,count,pass})=>(
                <div key={label} style={{background:"#0d1929",border:`1px solid ${pass?CORP_COLORS.green+"44":CORP_COLORS.red+"44"}`,borderRadius:8,padding:"12px 16px"}}>
                  ...
                  <div style={{fontSize:20,fontWeight:700,color:pass?CORP_COLORS.green:CORP_COLORS.red,fontFamily:"monospace"}}>{count} pts</div>
